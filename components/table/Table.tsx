...
function Table({ data }: { data: Array<any> }) {
  ...
  const handleUserDelete = async (user_id: string) => {
    if (user_id) {
      const data = await axios
        .post("https://marquis-backend.onrender.com/user/deleteUser", {
          user_id,
        })
        .then((response) => {
  ...
  const handleUpdateUser = async () => {
    if (formData) {
      const data = await axios
        .post("https://marquis-backend.onrender.com/user/updateUser", formData)
        .then((response) => {
          console.log(response);
          fetchUsers()
            .then((p) =>
  ...
  const handleAddNewUser = async () => {
    ...
    if (addNewUser) {
      const data = await axios
        .post("https://marquis-backend.onrender.com/user/addUser", addNewUser)
        .then((response) => {
          console.log(response);
          fetchUsers()
            .then((p) =>
  ...
  return (
    <div className="overflow-x-auto w-full ">
      <div className="flex w-full mb-4 justify-end">
        <label htmlFor="adduser-modal" className="btn btn-primary">
          Add User
        </label>
      </div>
                ...
                <tr key={key}>
                  ...
                  <td className="flex space-x-1">
                    <button
                      ...
                      onClick={() => {
                        handleUserDelete(value.user_id);
                      }}
                    >
                      Delete
                    </button>
